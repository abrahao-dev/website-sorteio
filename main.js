...
const moonIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-slate-400"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>`;
...
function showModal(title, message, buttons) {
    ...
    modalMessage.innerHTML = message; // Usando innerHTML para permitir tags
    modalButtons.innerHTML = '';

    buttons.forEach(btnInfo => {
        const button = document.createElement('button');
        button.textContent = btnInfo.text;
        button.className = `font-bold py-2 px-4 rounded-lg transition ${btnInfo.className || 'bg-gray-500 hover:bg-gray-600'}`;
        button.onclick = () => {
            closeModal();
...
function saveHistoryToLocalStorage() {
    try {
        ...
        showModal("Erro", "Não foi possível salvar o histórico. O armazenamento do navegador pode estar cheio ou desativado.", [{text: "OK"}]);
    }
}
...
function performDraw() {
    ...
    if (allNames.length === 0) {
        showModal("Atenção", "Por favor, insira nomes para o sorteio.", [{text: 'OK', className: 'bg-indigo-600 hover:bg-indigo-700'}]);
        return;
    }
    ...
    if (availableNames.length === 0) {
        showModal("Fim do Sorteio", "Não há nomes disponíveis. Verifique se todos já foram sorteados no modo 'Não repetir'.", [{text: 'OK', className: 'bg-indigo-600 hover:bg-indigo-700'}]);
        return;
    }
    if (quantityToDraw > availableNames.length) {
        showModal("Atenção", `Você quer sortear ${quantityToDraw} nome(s), mas só há ${availableNames.length} disponível(is).`, [{text: 'OK', className: 'bg-indigo-600 hover:bg-indigo-700'}]);
        return;
    }
...
function updateHistoryDisplay() {
    ...
    history.forEach((draw) => {
        ...
        drawTitle.className = "history-id text-sm mb-2 border-b pb-1";
        ...
        draw.winners.forEach(winner => {
            ...
            const time = draw.timestamp.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });

            const historyItem = `
                <div class="history-item p-3 rounded-md flex items-center gap-3">
                    <span class="text-lg font-semibold">${winnerCount}º</span>
            ...
            `;
            drawGroup.innerHTML += historyItem;
        })
...
clearHistoryButton.addEventListener('click', () => {
    showModal(
        ...
        [
            { text: 'Sim, apagar tudo', className: 'bg-red-600 hover:bg-red-700 text-white', onClick: () => {
                history = [];
                saveHistoryToLocalStorage();
                updateHistoryDisplay();
            }},
            { text: 'Cancelar', className: 'bg-gray-500 hover:bg-gray-600 text-white' }
        ]
    );
});
...
howItWorksLink.addEventListener('click', (e) => {
    ...
    `;
    showModal("Como o Sorteador Funciona", content, [{text: 'Entendi!', className: 'bg-indigo-600 hover:bg-indigo-700 text-white'}]);
});